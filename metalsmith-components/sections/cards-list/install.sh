...
# Installation script for cards-list v0.0.1
# Content Hash: 5ceae028f6cbd0fa

set -e

echo "🔧 Installing cards-list v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/cards-list"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/cards-list"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "5ceae028f6cbd0fa" ]; then
    echo "✓ cards-list v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading cards-list (content changed)"
  fi
fi
...
# Check for manual-card in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/manual-card/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/manual-card/manifest.json" ]; then
  MISSING_DEPS+=("manual-card")
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/cards-list.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/cards-list.css" "$TARGET_DIR/"
