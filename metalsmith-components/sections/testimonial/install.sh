...
# Installation script for testimonial v0.0.1
# Content Hash: 3443f07393a15149

set -e

echo "🔧 Installing testimonial v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/testimonial"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/testimonial"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "3443f07393a15149" ]; then
    echo "✓ testimonial v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading testimonial (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/testimonial.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/testimonial.css" "$TARGET_DIR/"
