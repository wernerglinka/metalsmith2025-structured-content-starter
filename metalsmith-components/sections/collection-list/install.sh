...
# Installation script for collection-list v0.0.1
# Content Hash: 8acc0e08f161e7cd

set -e

echo "🔧 Installing collection-list v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/collection-list"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/collection-list"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "8acc0e08f161e7cd" ]; then
    echo "✓ collection-list v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading collection-list (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/collection-list.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/collection-list.css" "$TARGET_DIR/"
