...
# Installation script for maps v0.0.1
# Content Hash: d7b7785c6e0f9c90

set -e

echo "🔧 Installing maps v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/maps"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/maps"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "d7b7785c6e0f9c90" ]; then
    echo "✓ maps v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading maps (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/maps.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/maps.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/maps.js" "$TARGET_DIR/"
