...
# Installation script for audio-only v0.0.1
# Content Hash: 6d48fafa1d71cd5c

set -e

echo "🔧 Installing audio-only v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/audio-only"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/audio-only"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "6d48fafa1d71cd5c" ]; then
    echo "✓ audio-only v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading audio-only (content changed)"
  fi
fi
...
# Check for audio in both partials and sections
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/audio-only.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/audio-only.css" "$TARGET_DIR/"
