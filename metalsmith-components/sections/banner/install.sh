...
# Installation script for banner v0.0.1
# Content Hash: 49b9e43e2e836687

set -e

echo "🔧 Installing banner v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/banner"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/banner"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "49b9e43e2e836687" ]; then
    echo "✓ banner v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading banner (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/banner.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/banner.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/banner.js" "$TARGET_DIR/"
