...
# Installation script for search-only v0.0.1
# Content Hash: 639c399544deb194

set -e

echo "🔧 Installing search-only v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/search-only"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/search-only"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "639c399544deb194" ]; then
    echo "✓ search-only v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading search-only (content changed)"
  fi
fi
...
# Check for search in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/search/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/search/manifest.json" ]; then
  MISSING_DEPS+=("search")
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/search-only.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/search-only.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/search-only.js" "$TARGET_DIR/"
