...
# Installation script for header v0.0.1
# Content Hash: 5979a1c0383bb580

set -e

echo "🔧 Installing header v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/header"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/header"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "5979a1c0383bb580" ]; then
    echo "✓ header v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading header (content changed)"
  fi
fi
...
# Check for branding in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/branding/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/branding/manifest.json" ]; then
  MISSING_DEPS+=("branding")
fi
# Check for navigation in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/navigation/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/navigation/manifest.json" ]; then
  MISSING_DEPS+=("navigation")
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/header.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/header.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/header.js" "$TARGET_DIR/"
