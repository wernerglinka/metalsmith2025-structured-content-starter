...
# Installation script for image-compare v0.0.1
# Content Hash: cc61e2d3710544d8

set -e

echo "🔧 Installing image-compare v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/image-compare"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/image-compare"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "cc61e2d3710544d8" ]; then
    echo "✓ image-compare v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading image-compare (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/image-compare.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/image-compare.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/image-compare.js" "$TARGET_DIR/"
