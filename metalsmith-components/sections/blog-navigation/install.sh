...
# Installation script for blog-navigation v0.0.1
# Content Hash: 08795c07234829ee

set -e

echo "🔧 Installing blog-navigation v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/blog-navigation"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/blog-navigation"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "08795c07234829ee" ]; then
    echo "✓ blog-navigation v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading blog-navigation (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/blog-navigation.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/blog-navigation.css" "$TARGET_DIR/"
