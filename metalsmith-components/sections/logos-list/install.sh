...
# Installation script for logos-list v0.0.1
# Content Hash: c0a7683ed7a2162e

set -e

echo "🔧 Installing logos-list v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/logos-list"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/logos-list"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "c0a7683ed7a2162e" ]; then
    echo "✓ logos-list v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading logos-list (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/logos-list.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/logos-list.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/logos-list.js" "$TARGET_DIR/"
