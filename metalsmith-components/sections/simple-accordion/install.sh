...
# Installation script for simple-accordion v0.0.1
# Content Hash: 311ba6349ad31eb9

set -e

echo "🔧 Installing simple-accordion v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/simple-accordion"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/simple-accordion"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "311ba6349ad31eb9" ]; then
    echo "✓ simple-accordion v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading simple-accordion (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/simple-accordion.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/simple-accordion.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/simple-accordion.js" "$TARGET_DIR/"
