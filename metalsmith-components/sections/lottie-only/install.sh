...
# Installation script for lottie-only v0.0.1
# Content Hash: 24a889db978370e7

set -e

echo "🔧 Installing lottie-only v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/lottie-only"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/lottie-only"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "24a889db978370e7" ]; then
    echo "✓ lottie-only v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading lottie-only (content changed)"
  fi
fi
...
# Check for lottie in both partials and sections
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/lottie-only.njk" "$TARGET_DIR/"
