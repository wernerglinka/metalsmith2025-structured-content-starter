...
# Installation script for hero-slider v0.0.1
# Content Hash: 38065cb732c60031

set -e

echo "🔧 Installing hero-slider v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/hero-slider"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/hero-slider"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "38065cb732c60031" ]; then
    echo "✓ hero-slider v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading hero-slider (content changed)"
  fi
fi
...
# Check for video in both partials and sections
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/hero-slider.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/hero-slider.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/hero-slider.js" "$TARGET_DIR/"
