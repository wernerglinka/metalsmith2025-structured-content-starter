...
# Installation script for video-only v0.0.1
# Content Hash: 7b0c1a32381b7732

set -e

echo "🔧 Installing video-only v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/video-only"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/video-only"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "7b0c1a32381b7732" ]; then
    echo "✓ video-only v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading video-only (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/video-only.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/video-only.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/video-only.js" "$TARGET_DIR/"
