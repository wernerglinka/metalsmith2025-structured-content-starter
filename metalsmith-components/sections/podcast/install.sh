...
# Installation script for podcast v0.0.1
# Content Hash: b966aad8bb1467f8

set -e

echo "🔧 Installing podcast v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/podcast"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/podcast"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "b966aad8bb1467f8" ]; then
    echo "✓ podcast v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading podcast (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/podcast.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/podcast.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/podcast.js" "$TARGET_DIR/"
