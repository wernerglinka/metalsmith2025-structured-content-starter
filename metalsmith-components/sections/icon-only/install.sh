...
# Installation script for icon-only v0.0.1
# Content Hash: 4e909040f2318259

set -e

echo "🔧 Installing icon-only v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/icon-only"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/icon-only"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "4e909040f2318259" ]; then
    echo "✓ icon-only v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading icon-only (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/icon-only.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/icon-only.css" "$TARGET_DIR/"
