...
# Installation script for compound v0.0.1
# Content Hash: c5bcd96cf73a176a

set -e

echo "🔧 Installing compound v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/compound"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/compound"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "c5bcd96cf73a176a" ]; then
    echo "✓ compound v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading compound (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/compound.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/compound.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/compound.js" "$TARGET_DIR/"
