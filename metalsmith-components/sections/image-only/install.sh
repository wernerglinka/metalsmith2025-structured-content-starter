...
# Installation script for image-only v0.0.1
# Content Hash: e9faf2b7004b6b21

set -e

echo "🔧 Installing image-only v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/image-only"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/image-only"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "e9faf2b7004b6b21" ]; then
    echo "✓ image-only v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading image-only (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/image-only.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/image-only.css" "$TARGET_DIR/"
