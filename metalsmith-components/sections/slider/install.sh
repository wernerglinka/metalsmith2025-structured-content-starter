...
# Installation script for slider v0.0.1
# Content Hash: c884060a958dfc47

set -e

echo "🔧 Installing slider v0.0.1..."
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "c884060a958dfc47" ]; then
    echo "✓ slider v0.0.1 already installed (no changes)"
    exit 0
  else
...
# Check for slider-pagination in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/slider-pagination/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/slider-pagination/manifest.json" ]; then
  MISSING_DEPS+=("slider-pagination")
fi
