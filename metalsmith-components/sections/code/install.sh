...
# Installation script for code v0.0.1
# Content Hash: 4a23b03b27e4add8

set -e

echo "🔧 Installing code v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/code"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/code"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "4a23b03b27e4add8" ]; then
    echo "✓ code v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading code (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/code.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/code.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/code.js" "$TARGET_DIR/"
