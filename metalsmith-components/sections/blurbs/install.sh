...
# Installation script for blurbs v0.0.1
# Content Hash: 50cc9af0a06bd980

set -e

echo "🔧 Installing blurbs v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/blurbs"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/blurbs"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "50cc9af0a06bd980" ]; then
    echo "✓ blurbs v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading blurbs (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/blurbs.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/blurbs.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/blurbs.js" "$TARGET_DIR/"
