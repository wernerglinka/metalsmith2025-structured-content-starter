...
# Installation script for calendar v0.0.1
# Content Hash: 760d96f454a12a25

set -e

echo "🔧 Installing calendar v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/calendar"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/calendar"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "760d96f454a12a25" ]; then
    echo "✓ calendar v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading calendar (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/calendar.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/calendar.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/calendar.js" "$TARGET_DIR/"
