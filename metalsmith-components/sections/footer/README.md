# Footer Section

**Version:** 0.0.1
**Content Hash:** 3f15f959c45cbd40

## Features

- Includes custom styles

## Installation

### Automated Installation

```bash
./install.sh
```

The install script will:
- Detect your Metalsmith project
- Read paths from `metalsmith-components.config.json` (if present)
- Check for existing versions
- Verify dependencies
- Copy files to the correct locations

**Note:** Component paths can be customized via `metalsmith-components.config.json` in your project root. See the bundle README for details.

### Manual Installation

Copy the component files to your Metalsmith project:

```bash
cp footer.njk your-project/lib/layouts/components/sections/footer/
cp footer.css your-project/lib/layouts/components/sections/footer/
cp manifest.json your-project/lib/layouts/components/sections/footer/
```

## Usage

Add the footer section to your page frontmatter:

### Basic Example

Minimal footer configuration

```yaml
sections:
  - sectionType: footer
    text:
      title: Example Title
      prose: Example content
```

## More Information

For complete documentation and live examples, visit:
https://metalsmith-components.netlify.app/library/footer/

