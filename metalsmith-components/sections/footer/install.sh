...
# Installation script for footer v0.0.1
# Content Hash: 3f15f959c45cbd40

set -e

echo "🔧 Installing footer v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/footer"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/footer"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "3f15f959c45cbd40" ]; then
    echo "✓ footer v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading footer (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/footer.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/footer.css" "$TARGET_DIR/"
