...
# Installation script for blog-author v0.0.1
# Content Hash: b13b8578ae4ebc5a

set -e

echo "🔧 Installing blog-author v0.0.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/blog-author"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/blog-author"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "b13b8578ae4ebc5a" ]; then
    echo "✓ blog-author v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading blog-author (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/blog-author.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/blog-author.css" "$TARGET_DIR/"
