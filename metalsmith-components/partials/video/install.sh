...
# Installation script for video v0.0.1
# Content Hash: 7b6d99f183ef4238

set -e

echo "🔧 Installing video v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/video"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/video"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "7b6d99f183ef4238" ]; then
    echo "✓ video v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading video (content changed)"
  fi
fi
...
# Check for overlay in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/overlay/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/overlay/manifest.json" ]; then
  MISSING_DEPS+=("overlay")
fi
# Check for image in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/image/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/image/manifest.json" ]; then
  MISSING_DEPS+=("image")
fi
# Check for icon in both partials and sections
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/video.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/video.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/video.js" "$TARGET_DIR/"
cp "$COMPONENT_DIR/manifest.json" "$TARGET_DIR/"

# Copy modules
mkdir -p "$TARGET_DIR/modules/providers"
mkdir -p "$TARGET_DIR/modules/helpers"
cp -r "$COMPONENT_DIR/modules/"* "$TARGET_DIR/modules/"
