...
# Installation script for author-date v0.0.1
# Content Hash: 31a6b688afc25f68

set -e

echo "🔧 Installing author-date v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/author-date"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/author-date"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "31a6b688afc25f68" ]; then
    echo "✓ author-date v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading author-date (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/author-date.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/author-date.css" "$TARGET_DIR/"
