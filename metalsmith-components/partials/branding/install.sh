...
# Installation script for branding v0.0.1
# Content Hash: 44006d495b11648f

set -e

echo "🔧 Installing branding v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/branding"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/branding"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "44006d495b11648f" ]; then
    echo "✓ branding v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading branding (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/branding.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/branding.css" "$TARGET_DIR/"
