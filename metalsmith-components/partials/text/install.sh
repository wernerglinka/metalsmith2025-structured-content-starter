...
# Installation script for text v0.0.1
# Content Hash: 41ecad7325b5c371

set -e

echo "🔧 Installing text v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/text"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/text"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "41ecad7325b5c371" ]; then
    echo "✓ text v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading text (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/text.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/text.css" "$TARGET_DIR/"
