...
# Installation script for audio v0.0.1
# Content Hash: 110aa8502fa63012

set -e

echo "🔧 Installing audio v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/audio"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/audio"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "110aa8502fa63012" ]; then
    echo "✓ audio v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading audio (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/audio.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/audio.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/audio.js" "$TARGET_DIR/"
