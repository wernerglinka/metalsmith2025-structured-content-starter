...
# Installation script for ctas v0.0.1
# Content Hash: 73b6fb13ee63d360

set -e

echo "🔧 Installing ctas v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/ctas"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/ctas"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "73b6fb13ee63d360" ]; then
    echo "✓ ctas v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading ctas (content changed)"
  fi
fi
...
# Check for button in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/button/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/button/manifest.json" ]; then
  MISSING_DEPS+=("button")
fi
# Check for text-link in both partials and sections
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/text-link/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/text-link/manifest.json" ]; then
  MISSING_DEPS+=("text-link")
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/ctas.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/ctas.css" "$TARGET_DIR/"
