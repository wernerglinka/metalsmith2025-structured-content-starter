...
# Installation script for dark-light-theme-switcher v0.0.1
# Content Hash: 7cb96caf563b3b18

set -e

echo "🔧 Installing dark-light-theme-switcher v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/dark-light-theme-switcher"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/dark-light-theme-switcher"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "7cb96caf563b3b18" ]; then
    echo "✓ dark-light-theme-switcher v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading dark-light-theme-switcher (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/dark-light-theme-switcher.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/dark-light-theme-switcher.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/dark-light-theme-switcher.js" "$TARGET_DIR/"
