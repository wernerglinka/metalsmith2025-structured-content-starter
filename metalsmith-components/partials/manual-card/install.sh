...
# Installation script for manual-card v0.0.1
# Content Hash: 112c11765d684d88

set -e

echo "🔧 Installing manual-card v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/manual-card"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/manual-card"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "112c11765d684d88" ]; then
    echo "✓ manual-card v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading manual-card (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/manual-card.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/manual-card.css" "$TARGET_DIR/"
