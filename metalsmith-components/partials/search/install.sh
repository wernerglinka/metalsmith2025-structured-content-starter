...
# Installation script for search v0.0.1
# Content Hash: 02b7867ad13e1ca1

set -e

echo "🔧 Installing search v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/search"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/search"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "02b7867ad13e1ca1" ]; then
    echo "✓ search v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading search (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/search.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/search.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/search.js" "$TARGET_DIR/"
