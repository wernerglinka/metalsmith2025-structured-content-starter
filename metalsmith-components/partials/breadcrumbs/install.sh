...
# Installation script for breadcrumbs v0.0.1
# Content Hash: d8784086e18a519d

set -e

echo "🔧 Installing breadcrumbs v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/breadcrumbs"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/breadcrumbs"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "d8784086e18a519d" ]; then
    echo "✓ breadcrumbs v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading breadcrumbs (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/breadcrumbs.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/breadcrumbs.css" "$TARGET_DIR/"
