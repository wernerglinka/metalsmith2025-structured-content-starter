...
# Installation script for flip-card v0.0.1
# Content Hash: 28b772cfee27da9d

set -e

echo "🔧 Installing flip-card v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/flip-card"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/flip-card"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "28b772cfee27da9d" ]; then
    echo "✓ flip-card v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading flip-card (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/flip-card.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/flip-card.css" "$TARGET_DIR/"
cp "$COMPONENT_DIR/flip-card.js" "$TARGET_DIR/"
