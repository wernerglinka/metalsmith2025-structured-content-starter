...
# Installation script for image v0.0.1
# Content Hash: 4ab48df8b296709b

set -e

echo "🔧 Installing image v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/image"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/image"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "4ab48df8b296709b" ]; then
    echo "✓ image v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading image (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/image.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/image.css" "$TARGET_DIR/"
