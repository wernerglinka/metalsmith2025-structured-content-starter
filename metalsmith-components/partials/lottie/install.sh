...
# Installation script for lottie v0.0.1
# Content Hash: 1142b9f0814c0f79

set -e

echo "🔧 Installing lottie v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/lottie"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/lottie"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "1142b9f0814c0f79" ]; then
    echo "✓ lottie v0.0.1 already installed (no changes)"
    exit 0
  else
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/lottie.njk" "$TARGET_DIR/"
