...
# Installation script for collection-pagination v0.0.1
# Content Hash: be6fc67435617537

set -e

echo "🔧 Installing collection-pagination v0.0.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/collection-pagination"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/collection-pagination"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "be6fc67435617537" ]; then
    echo "✓ collection-pagination v0.0.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading collection-pagination (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/collection-pagination.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/collection-pagination.css" "$TARGET_DIR/"
