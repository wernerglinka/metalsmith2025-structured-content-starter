...
# Installation script for commons v1.4.0
# Content Hash: 3786c1bdf0068b14
...
echo "🔧 Installing commons v1.4.0..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/commons"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/commons"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "3786c1bdf0068b14" ]; then
    echo "✓ commons v1.4.0 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading commons (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/commons.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/commons.css" "$TARGET_DIR/"
