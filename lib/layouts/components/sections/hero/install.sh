...
# Installation script for hero v1.4.0
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/hero"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/hero"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "d2015e3a200be7a3" ]; then
    echo "✓ hero v1.4.0 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading hero (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/hero.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/hero.css" "$TARGET_DIR/"
