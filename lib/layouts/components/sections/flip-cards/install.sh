...
# Installation script for flip-cards v1.3.1
...
echo "🔧 Installing flip-cards v1.3.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/flip-cards"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/flip-cards"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "eb2bce588b89624c" ]; then
    echo "✓ flip-cards v1.3.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading flip-cards (content changed)"
  fi
fi
...
# Check for flip-card
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/flip-card/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/flip-card/manifest.json" ]; then
  # Try to auto-install (most dependencies are partials)
  if ! install_dependency "flip-card" "partial"; then
    # Try as section if partial fails
    if ! install_dependency "flip-card" "section"; then
      FAILED_DEPS="$FAILED_DEPS flip-card"
    fi
  fi
else
  echo "  ✓ flip-card (already installed)"
fi
# Check for commons
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/flip-cards.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/flip-cards.css" "$TARGET_DIR/"
