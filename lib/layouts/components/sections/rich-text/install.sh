...
# Installation script for rich-text v1.3.1
# Content Hash: 215831031be9d3f2
...
echo "🔧 Installing rich-text v1.3.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/rich-text"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/rich-text"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "215831031be9d3f2" ]; then
    echo "✓ rich-text v1.3.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading rich-text (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/rich-text.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/rich-text.css" "$TARGET_DIR/"
