...
# Installation script for columns v1.3.1
# Content Hash: b0e6c76cec4da4a0
...
echo "🔧 Installing columns v1.3.1..."
...
if [ "section" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/columns"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/columns"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "b0e6c76cec4da4a0" ]; then
    echo "✓ columns v1.3.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading columns (content changed)"
  fi
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/columns.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/columns.css" "$TARGET_DIR/"
