...
# Installation script for iframe v1.5.0
# Content Hash: e008ed03dbf4e477
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/iframe"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/iframe"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "e008ed03dbf4e477" ]; then
    echo "✓ iframe v1.5.0 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading iframe (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/iframe.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/iframe.css" "$TARGET_DIR/"
