...
# Installation script for collection-card v1.3.1
# Content Hash: 0e53193ba293ccc1
...
echo "🔧 Installing collection-card v1.3.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/collection-card"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/collection-card"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "0e53193ba293ccc1" ]; then
    echo "✓ collection-card v1.3.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading collection-card (content changed)"
  fi
fi
...
# Check for author-date
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/author-date/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/author-date/manifest.json" ]; then
  # Try to auto-install (most dependencies are partials)
  if ! install_dependency "author-date" "partial"; then
    # Try as section if partial fails
    if ! install_dependency "author-date" "section"; then
      FAILED_DEPS="$FAILED_DEPS author-date"
    fi
  fi
else
  echo "  ✓ author-date (already installed)"
fi
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/collection-card.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/collection-card.css" "$TARGET_DIR/"
