...
# Installation script for disclosure v1.4.0
# Content Hash: 08467956a85a4c8c
...
echo "🔧 Installing disclosure v1.4.0..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/disclosure"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/disclosure"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "08467956a85a4c8c" ]; then
    echo "✓ disclosure v1.4.0 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading disclosure (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/disclosure.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/disclosure.css" "$TARGET_DIR/"
