...
# Installation script for button v1.3.1
# Content Hash: c40f61e99b9d0169
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/button"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/button"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "c40f61e99b9d0169" ]; then
    echo "✓ button v1.3.1 already installed (no changes)"
    exit 0
  else
...
# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/button.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/button.css" "$TARGET_DIR/"
