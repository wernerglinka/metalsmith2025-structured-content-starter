...
# Installation script for overlay v1.3.1
# Content Hash: 539d313b9367764b
...
echo "🔧 Installing overlay v1.3.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/overlay"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/overlay"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "539d313b9367764b" ]; then
    echo "✓ overlay v1.3.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading overlay (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/overlay.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/overlay.css" "$TARGET_DIR/"
