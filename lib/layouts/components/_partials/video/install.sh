...
# Installation script for video v1.3.1
# Content Hash: 6084619819b9d945
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "6084619819b9d945" ]; then
    echo "✓ video v1.3.1 already installed (no changes)"
    exit 0
  else
...
if [ ! -f "$COMPONENTS_BASE/$PARTIALS_DIR/overlay/manifest.json" ] && [ ! -f "$COMPONENTS_BASE/$SECTIONS_DIR/overlay/manifest.json" ]; then
  ...
  if ! install_dependency "overlay" "partial"; then
    # Try as section if partial fails
    if ! install_dependency "overlay" "section"; then
      FAILED_DEPS="$FAILED_DEPS overlay"
    fi
  fi
else
...
if [ -d "$COMPONENT_DIR/modules" ]; then
  cp -r "$COMPONENT_DIR/modules" "$TARGET_DIR/"
fi
