...
# Installation script for text-link v1.3.1
# Content Hash: 34ae81631df76735
...
echo "🔧 Installing text-link v1.3.1..."
...
if [ "partial" = "section" ]; then
  TARGET_DIR="$COMPONENTS_BASE/$SECTIONS_DIR/text-link"
else
  TARGET_DIR="$COMPONENTS_BASE/$PARTIALS_DIR/text-link"
fi
mkdir -p "$TARGET_DIR"
...
if [ -f "$TARGET_DIR/manifest.json" ]; then
  ...
  if [ "$EXISTING_HASH" = "34ae81631df76735" ]; then
    echo "✓ text-link v1.3.1 already installed (no changes)"
    exit 0
  else
    echo "📦 Upgrading text-link (content changed)"
  fi
fi

# Copy files
echo "Copying files..."
cp "$COMPONENT_DIR/text-link.njk" "$TARGET_DIR/"
cp "$COMPONENT_DIR/text-link.css" "$TARGET_DIR/"
